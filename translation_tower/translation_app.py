...
class TranslationApp:
    ...
    async def translate_web_anno_tsv_file(
        self,
        tsv: str,
        translated_tsv: str,
        source_lang: str,
        ...
        with open_web_anno_tsv(tsv) as f:
            ...
            for i, sentence in enumerate(f):
                job = TranslationJob(request_id, i)
                job.source.text = sentence.text
                job.source.language = source_lang
                job.source.annotations = list(map(lambda a: Annotation(
                    a.text, a.start, a.stop), sentence.annotations))
                job.target.language = target_lang
                job.translator.name = translator
                jobs.append(job)
